...
class ModuleCBOrderHistory extends ModuleIsotope
{
	 ...
	 * @var string
	 */
	protected $strTemplate = 'mod_cb_orderhistory';
	...
	public function generate()
	{
		if (TL_MODE == 'BE')
		{
			...
			$objTemplate->wildcard = '### COURSE BUILDER: ORDER HISTORY ###';
	...
	protected function compile()
	{
		...
		while( $objOrders->next() )
		{
			...
			// Get total number of items per order
			$arrResult = $this->Database->prepare("SELECT SUM(product_quantity) AS `totalitems` FROM `tl_iso_order_items` WHERE pid = ?")->limit(1)->execute($objOrders->id)->row();
			...
			(
				...
				'items'			=> count($arrResult) ? $arrResult['totalitems'] : $objOrders->items,
